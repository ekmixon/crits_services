...
def pcap_tcpdump(pcap_md5, form, analyst):
    ...
    if not pcap_data:
        return f"Could not get PCAP from GridFS: {pcap_md5}"

    # Use the filename if it's there, otherwise the md5.
    # This is used for the description of the carved sample.
    pcap_filename = pcap.filename or pcap_md5
    # Setup tcpdump arguments
    if cleaned_data['sequence']:
        flag_list.append("-S")
    if cleaned_data['timestamp']:
        flag_list.append(f"{cleaned_data['timestamp']}")
    if cleaned_data['verbose']:
        flag_list.append(f"{cleaned_data['verbose']}")
    if cleaned_data['data']:
        flag_list.append(f"{cleaned_data['data']}")
    # force -nN
    flag_list.append("-nN")
    # if we need to carve
    if cleaned_data['carve']:
        ...
        flag_list.extend(("-w", new_pcap.name))
    if cleaned_data['bpf']:
    ...
    temp_out.seek(0)
    tcpdump_output = ''.join(f"{line}" for line in iter(temp_out))
    temp_out.close()
    ...
    if cleaned_data['carve']:
        ...
        if len(new_pcap_data) > 24: # pcap-ng will change this.
            ...
            result = handle_pcap_file(
                ...
                org,
                user=analyst,
                description=f"{cleaned_data['bpf']} of {pcap_filename}",
                parent_id=pcap.id,
                parent_type="PCAP",
...
def pcap_pdml_html(pcap_md5, analyst):
    if not (pcap := PCAP.objects(md5=pcap_md5).first()):
        return "No PCAP found"
    ...
    if not pdml_obj:
        ...
        pcap_data = pcap.filedata.read()
        if not pcap_data:
            pdml_html = f"Could not get PCAP from GridFS: {pcap_md5}"
            return {'html': pdml_html}
        ...
        if tshark.returncode != 0:
            return {'html': f"{tshark_out}, {tshark_err}"}
        ...
        for d in settings.SERVICE_DIRS:
            try:
                file_dir = f"{d}/metacap_service"
                xsl_file = open(f'{file_dir}/pdml2html.xsl', 'r')
            except IOError:
                pass
        if not xsl_file:
        ...
        if save_pdml:
            if fn := put_file_gridfs(
                'tshark_pdml.html', pdml_html, collection=coll
            ):
                m = hashlib.md5()
                m.update(pdml_html)
                md5 = m.hexdigest()
