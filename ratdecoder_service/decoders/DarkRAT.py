...
def config(data):
    ...
    if len(raw_config) > 3:
        config_dict = {
            'Domain': raw_config[1][7:-1],
            'AutoRun': raw_config[2],
            'USB Spread': raw_config[3],
            'Hide Form': raw_config[4],
            'Msg Box Title': raw_config[6],
            'Msg Box Text': raw_config[7],
            'Timer Interval': raw_config[8],
        }
