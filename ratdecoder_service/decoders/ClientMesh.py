import string
from base64 import b64decode


def stringPrintable(line):
    return filter(lambda x: x in string.printable, line)

def first_split(data):
    splits = data.split('\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x7e')
    if len(splits) == 2:
        return splits[1]


def base64_deocde(b64_string):
    return b64decode(b64_string)

    
def conf_extract(coded_config):
    decoded_conf = base64_deocde(coded_config)
    split_list = decoded_conf.split('``')
    return list(split_list)


def process_config(raw_config):
    return {
        'Domain': raw_config[0],
        'Port': raw_config[1],
        'Password': raw_config[2],
        'CampaignID': raw_config[3],
        'MsgBoxFlag': raw_config[4],
        'MsgBoxTitle': raw_config[5],
        'MsgBoxText': raw_config[6],
        'Startup': raw_config[7],
        'RegistryKey': raw_config[8],
        'RegistryPersistance': raw_config[9],
        'LocalKeyLogger': raw_config[10],
        'VisibleFlag': raw_config[11],
        'Unknown': raw_config[12],
    }

def config(data):
    coded_config = first_split(data)
    raw_config = conf_extract(coded_config)
    return process_config(raw_config)